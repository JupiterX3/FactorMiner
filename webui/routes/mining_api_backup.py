        ...
        def run_mining_background():
            try:
                ...
                if total_algorithms == 0:
                    update_session_progress(session_id, 'factor_building', 0, '错误：未选择任何算法')
                    return
                
                # 构建因子
                all_factors = pd.DataFrame(index=data_result['data'].index)
                
                for i, algorithm_id in enumerate(selected_algorithms):
                    progress_percent = int((i / total_algorithms) * 80) + 10  # 10% - 90%
                    try:
                        update_session_progress(session_id, 'factor_building', progress_percent, f'正在执行算法: {algorithm_id}...')
                        ...
                        if not factors.empty:
                            ...
                            update_session_progress(session_id, 'factor_building', progress_percent + 5, f'算法 {algorithm_id} 完成，生成 {len(factors.columns)} 个因子')
                        else:
                            update_session_progress(session_id, 'factor_building', progress_percent + 5, f'算法 {algorithm_id} 未生成因子')
                            
                    except Exception as e:
                        print(f"算法 {algorithm_id} 执行失败: {e}")
                        update_session_progress(session_id, 'factor_building', progress_percent + 5, f'算法 {algorithm_id} 执行失败: {str(e)}')
                        continue
