...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    ...
    try:
        ...
        # 智能查找模型文件
        model_paths = [
            Path(__file__).parent.parent / "models" / "rolling_ml_factor.pkl",
            Path.cwd() / "factorlib" / "models" / "rolling_ml_factor.pkl",
            Path(__file__).parent.parent.parent / "models" / "rolling_ml_factor.pkl"
        ]
        ...
        print(f"计算rolling_ml_factor因子时出错: {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
