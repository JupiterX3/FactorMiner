...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    """
    计算ensemble_gradient_boosting因子
    ...
    try:
        ...
        # 智能查找模型文件
        model_paths = [
            Path(__file__).parent.parent / "models" / "ensemble_gradient_boosting.pkl",
            Path.cwd() / "factorlib" / "models" / "ensemble_gradient_boosting.pkl",
            Path(__file__).parent.parent.parent / "models" / "ensemble_gradient_boosting.pkl"
        ]
        ...
        print(f"计算ensemble_gradient_boosting因子时出错: {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
