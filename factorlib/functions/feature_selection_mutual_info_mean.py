...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    """
    计算feature_selection_mutual_info_mean因子
    ...
    try:
        ...
        # 智能查找模型文件
        model_paths = [
            Path(__file__).parent.parent / "models" / "feature_selection_mutual_info_mean.pkl",
            Path.cwd() / "factorlib" / "models" / "feature_selection_mutual_info_mean.pkl",
            Path(__file__).parent.parent.parent / "models" / "feature_selection_mutual_info_mean.pkl"
        ]
        ...
        print(f"计算feature_selection_mutual_info_mean因子时出错: {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
