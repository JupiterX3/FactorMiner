...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    ...
    try:
        ...
        # 智能查找模型文件
        model_paths = [
            Path(__file__).parent.parent / "models" / "pca_component_1.pkl",
            Path.cwd() / "factorlib" / "models" / "pca_component_1.pkl",
            Path(__file__).parent.parent.parent / "models" / "pca_component_1.pkl"
        ]
        ...
        print(f"计算pca_component_1因子时出错: {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
