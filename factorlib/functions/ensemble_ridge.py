...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    ...
    try:
        ...
        # 智能查找模型文件
        model_paths = [
            Path(__file__).parent.parent / "models" / "ensemble_ridge.pkl",
            Path.cwd() / "factorlib" / "models" / "ensemble_ridge.pkl",
            Path(__file__).parent.parent.parent / "models" / "ensemble_ridge.pkl"
        ]
        ...
        print(f"计算ensemble_ridge因子时出错: {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
