...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    """
    计算feature_selection_f_regression_weighted因子
    ...
    try:
        ...
        # 智能查找模型文件
        model_paths = [
            Path(__file__).parent.parent / "models" / "feature_selection_f_regression_weighted.pkl",
            Path.cwd() / "factorlib" / "models" / "feature_selection_f_regression_weighted.pkl",
            Path(__file__).parent.parent.parent / "models" / "feature_selection_f_regression_weighted.pkl"
        ]
        ...
        print(f"计算feature_selection_f_regression_weighted因子时出错: {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
