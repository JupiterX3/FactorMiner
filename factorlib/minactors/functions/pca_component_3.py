...
def calculate(data: pd.DataFrame, **kwargs) -> pd.Series:
    ...
    try:
        ...
        if artifact_file is None:
            print(f"未找到模型文�? {FACTOR_NAME}.pkl")
        ...
        # 处理NaN�?- PCA不接受NaN�?
        # 方法1：前向填充，然后后向填充
        X_cleaned = X.ffill().bfill()
        
        # 方法2：如果仍有NaN值，�?填充
        if X_cleaned.isna().any().any():
            print(f"警告：仍有NaN值，�?填充")
            X_cleaned = X_cleaned.fillna(0)
        
        # 方法3：如果某行全是NaN，删除该�?
        valid_rows = ~X_cleaned.isna().all(axis=1)
        if not valid_rows.all():
            print(f"删除包含全NaN的行，从{len(X_cleaned)}行减少到{valid_rows.sum()}�?)
        ...
        print(f"计算pca_component_3因子时出�? {e}")
        import traceback
        traceback.print_exc()
    return pd.Series(index=data.index, dtype=float)
